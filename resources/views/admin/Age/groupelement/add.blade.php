...
<head>
    ...
    <title>SARC CMS | Add Group Element</title>
                ...
                <div class="container-fluid">
                    ...
                    <div class="card card-info">
                        <div class="card-header">
                            <h3 class="card-title">Add Group Element Information</h3>
                            <a href="{{ url('/Agegroupelement/all') }}" class="btn btn-danger"
                                style="float: right;">back</a>
                        </div>
                        <div class="card-body">
                            <form method="POST" action="{{ route('admin.Agegroupelement.store') }}">
                                @csrf
                                <!-- Color Picker -->
                                ...
                                <div class="form-group">
                                    <label>Group</label>
                                    <select name="group_id" id="select_group" class="form-select my-colorpicker1"
                                        required foccus>
                                        <option value="" disabled selected>Please Select Group</option>
                                        @foreach ($Agecategorygroup as $Agecategorygroups)
                                            <option value="{{ $Agecategorygroups->id }}">{{ $Agecategorygroups->en_name }}</option>
                                        @endforeach
                                    </select>
                                    @error('cat_id')
                                        <small class="form-text text-danger">{{ $message }}</small>
