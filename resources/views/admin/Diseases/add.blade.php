...
<head>
  ...
  <title>SARC CMS | Add Diseases</title>
        ...
        <div class="container-fluid">
          ...
          <div class="card card-info">
            <div class="card-header">
              <h3 class="card-title">Add Diseases Information</h3>
            ...
            <div class="card-body">
              <form method="POST" action="{{ route('admin.diseases.store') }}">
                ...
                <div class="form-group">
                    ...
                    <select name="cat_id" id="select_category" class="form-select my-colorpicker1"
                        required foccus>
                        <option value="" disabled selected>Please Select Category</option>
                        @foreach ($Diseasecats as $Diseasecat)
                            <option value="{{ $Diseasecat->id }}">{{ $Diseasecat->en_name }}</option>
                        @endforeach
                    </select>
                    @error('cat_id')
                        <small class="form-text text-danger">{{ $message }}</small>
